...
assert.match(source, /opts\.dateOnly === true/, 'date-only wheels must be an explicit renderer mode');
assert.match(source, /if \(!dateOnly\) drawHouseGrid/, 'date-only wheels must not draw house cusps');
assert.match(chartPageSource, /dateOnly: dateOnly/, 'chart page must ask for a date wheel when time is unknown');

...
const fails = [];
if (!/ap-eclipse-live-v834\.js\?v=868/.test(eclipseHtml)) fails.push('eclipse missing dedicated 3D module');
...
if (!/ap-eclipse-contact-v835\.js\?v=868/.test(eclipseHtml)) fails.push('eclipse missing contact controller');
...
if (!/The Moon crossed the Sun/i.test(eclipseHtml)) fails.push('eclipse missing post-event brand line');
if (/Watch the shadow arrive/i.test(eclipseHtml)) fails.push('eclipse still uses pre-event launch brand line');
if (!/ap-eclipse-next/.test(eclipseHtml)) fails.push('eclipse missing next-eclipses hub strip');
if (!/Date\.now\(\) >= EVENT_MS/.test(liveJs)) fails.push('eclipse live module does not default to greatest replay after the event');
...
if (!/const V\s*=\s*["']ap-v868["']/.test(sw)) fails.push('SW tip is not exactly ap-v868');
...
if (!/loadEditionContext/.test(contactJs)) fails.push('contact controller missing pending-edition restore');
if (!/productId:\s*'3ZwFjg0IW702KvJ5s97QuQ=='/.test(readFileSync('website/js/gumroad-unlock.js', 'utf8'))) {
  fails.push('eclipse unlock productId is not the live Gumroad id');
}

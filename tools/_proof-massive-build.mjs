...
ok(/js\/ap-nav-model\.js\?v=861/.test(index) && !/ap-nav-model-v834/.test(index), 'index uses the one canonical navigation model');
...
ok(/js\/chart-page\.js\?v=861/.test(chart), 'chart loads the current calculation controller');
...
ok(/ap-eclipse-live-v834\.js\?v=861/.test(eclipse), 'eclipse loads the dedicated 3D instrument');
...
ok(versionMatch && versionMatch[1] === 'ap-v861', 'SW release tip ' + (versionMatch && versionMatch[1]));

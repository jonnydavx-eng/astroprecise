...
(function () {
  ...
  function quietFollowLink() {
    return '<a class="ap-social-row__when-live" href="links.html">Follow when live</a>';
  }

  function liveChannels(S) {
    return SOCIAL_ORDER.filter(function (o) { return isLiveSocial(S[o[0]]); });
  }
  ...
  function injectStyles() {
    ...
    st.textContent = [
      '.legal-footer,.ap-legal-footer{max-width:720px;margin:0 auto;padding:0 0 48px;text-align:center;}',
      '.ap-social-row{padding:1.25rem 0 0;}',
      '.ap-social-row__head{margin-bottom:0.75rem;}',
      '.ap-social-row__label{display:block;font-family:Inter,system-ui,sans-serif;font-size:0.62rem;letter-spacing:0.18em;text-transform:uppercase;color:#C9D6E3;}',
      ...
      'background:rgba(10,20,36,0.78);border:1px solid rgba(139,169,255,0.2);display:inline-flex;',
      'align-items:center;justify-content:center;color:#93A8BF;text-decoration:none;cursor:pointer;padding:0;box-sizing:border-box;}',
      '.ap-social-icon__svg{width:18px;height:18px;}',
      '.ap-social-icon--live:hover{color:#A5BCFF;border-color:rgba(165,188,255,0.48);}',
      '.ap-social-row__when-live{display:inline-block;font-family:Inter,system-ui,sans-serif;font-size:0.62rem;letter-spacing:0.12em;text-transform:uppercase;color:#93A8BF;text-decoration:none;}',
      'a.ap-social-row__when-live:hover{color:#C9D6E3;text-decoration:underline;text-underline-offset:2px;}',
    ].join('');
  ...
  function renderSocialRow(host) {
    if (!host) return;
    var S = cfg();
    var channels = liveChannels(S);
    if (!channels.length) {
      host.innerHTML = quietFollowLink();
      return;
    }
    ...
    if (!icons) return;
    icons.innerHTML = channels.map(function (o) {

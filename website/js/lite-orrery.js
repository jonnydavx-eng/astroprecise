...
(function () {
  ...
  function drawStarField(ctx, W, H, now) {
    ...
    stars.forEach(function (st) {
      ...
      if (award) {
        ...
        ctx.fillStyle = brass
          ? 'rgba(216, 180, 106, ' + Math.min(1, alpha * 1.08) + ')'
  ...
  function drawPoster(now) {
    ...
    if (award) {
      ...
      edgeGlow.addColorStop(0.55, 'rgba(216, 180, 106, 0.06)');
    ...
    for (var s = 0; s < 12; s++) {
      ...
      ctx.strokeStyle = active
        ? (award ? 'rgba(216, 180, 106, 0.88)' : 'rgba(232, 201, 106, 0.78)')
        : (award ? 'rgba(216, 180, 106, 0.26)' : 'rgba(201, 162, 39, 0.18)');
      ctx.lineWidth = active ? 4 : 1.4;
      ctx.stroke();
    }

    ctx.strokeStyle = award ? 'rgba(216, 180, 106, 0.22)' : 'rgba(150, 190, 240, 0.22)';
    ...
    if (award) {
      ...
      coronaG.addColorStop(0.72, 'rgba(216, 180, 106, ' + (0.05 * sunPulse).toFixed(3) + ')');
      coronaG.addColorStop(1, 'rgba(216, 180, 106, 0)');
    ...
    if (award) {
      ...
      sunG.addColorStop(0.55, 'rgba(216, 180, 106, 0.52)');
    ...
    PLANETS.forEach(function (p, i) {
      ...
      if (hoverId === p.id) {
        ...
        if (award) {
          hg.addColorStop(0, 'rgba(216, 180, 106, ' + (0.34 * pulse + 0.18).toFixed(3) + ')');
          hg.addColorStop(1, 'rgba(216, 180, 106, 0)');
        } else {
    ...
    if (award) {
      ctx.strokeStyle = 'rgba(216, 180, 106, 0.14)';
      ctx.lineWidth = 0.8;
      for (var t = 0; t < 12; t++) {

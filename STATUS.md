...
**State:** Live & current. Latest includes **sw `ap-v90`** (full site audit fixes P0–P2). Pushed to `main`; auto-deploys via `deploy-pages.yml` → `gh-pages`. To see changes: **hard-refresh / reinstall the PWA** (the service worker caches the shell; each `ap-vNN` bump replaces it).
...
*Updated 2026-06-14 (ap-v90). Full history: AGENT-HANDOFF.md + CLAUDE.md here · roadmap: ~/.claude/plans/elegant-popping-zebra.md*
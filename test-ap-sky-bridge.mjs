...
// APDeepLink normalizeMoment bare ISO
{
  ...
  ok('live-now link needs no public-event marker', nowLink === 'index.html#m=now&focus=earth', nowLink);
}
   ...
   stays in the fragment. Added 2026-08-09 with the leak fix. */
{
  ...
  ok('a life-moment freeze publishes no instant', lifeMoment.link === 'index.html#focus=earth', lifeMoment.link);
  ...
  ok('storage blocked → fallback keeps safe focus', fallback === 'index.html#focus=venus', fallback);

  const safeScale = DL.stashSkyLink({
    m: '1994-03-14T09:12:00.000Z',
    focus: 'mars',
    scale: 2,
    base: 'index.html#m=stale-private-value',
  });
  ok('fallback strips a stale base fragment', safeScale === 'index.html#focus=mars&scale=2', safeScale);

  const safeQuery = DL.stashSkyLink({
    m: '1994-03-14T09:12:00.000Z',
    focus: 'mars',
    base: 'index.html?nosw=1&m=stale-private-value#focus=earth',
  });
  ok('fallback removes stale moment query but keeps safe query', safeQuery === 'index.html?nosw=1#focus=mars', safeQuery);

  const doubleEncodedMoment = SB.buildLinkFromChart(
    { birthDate: '1994-03-14', birthTime: '09:12', tz: 'UTC' },
    { focus: 'venus', base: 'index.html?%256D=secret&nosw=1&campaign=drop' },
  );
  ok('double-encoded moment keys and unknown query fields are rejected',
    doubleEncodedMoment === 'index.html?nosw=1#focus=venus', doubleEncodedMoment);
  ...
  [
    ...
    ok('bridge rejects unsafe/custom base ' + base,
      safeDestination === 'index.html#focus=venus', safeDestination);
  });
  ...
  ok('deep-link helper independently rejects executable bases and encoded keys',
    directDeepLinkBase === 'index.html?lite=1#focus=mars', directDeepLinkBase);

  const inheritedFocus = DL.stashSkyLink({
    m: '1994-03-14T09:12:00.000Z',
    focus: '__proto__',
  });
  ok('prototype names are not accepted as focus slugs', inheritedFocus === 'index.html', inheritedFocus);

  const dateLink = SB.buildLinkFromDate('1994-03-14', { focus: 'earth' });
  ok('date-only birth route also carries NO moment', dateLink === 'index.html#focus=earth', dateLink);
  ...
  ctx.APDeepLink = {
    stashSkyLink: () => 'index.html#m=1994-03-14T09%3A12%3A00.000Z&focus=venus',
    buildSkyLink: () => 'index.html#m=1994-03-14T09%3A12%3A00.000Z&focus=venus',
  };
  const staleHelperLink = SB.buildLinkFromChart(
    { birthDate: '1994-03-14', birthTime: '09:12', tz: 'UTC' },
    { focus: 'venus', scale: 1 },
  );
  ok('stale helper result is rejected', staleHelperLink === 'index.html#focus=venus&scale=1', staleHelperLink);

  ctx.APDeepLink = {
    stashSkyLink: () => 'index.html#%6D=1994-03-14T09%3A12%3A00.000Z&focus=venus',
  };
  ...
  ok('percent-encoded stale moment key is rejected',
    encodedStaleHelperLink === 'index.html#focus=venus', encodedStaleHelperLink);

  ctx.APDeepLink = {
    stashSkyLink: () => 'index.html#focus=venus',
    buildFocusLink: () => 'index.html#%6D=1994-03-14T09%3A12%3A00.000Z&focus=venus',
  };
  ...
  ok('stale focus helper cannot reintroduce an encoded moment key',
    bothStaleHelpersLink === 'index.html#focus=venus', bothStaleHelpersLink);
  ...
  ok('stale helper cannot choose an unsafe destination',
    unsafeSchemeHelperLink === 'index.html#focus=venus', unsafeSchemeHelperLink);
  ...
  ok('missing helper fails closed', missingHelperLink === 'index.html#focus=saturn', missingHelperLink);
